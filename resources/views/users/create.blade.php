<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-slate-950">Tambah User</h2>
                <p class="mt-1 text-sm text-slate-500">Buat user baru dengan role akses.</p>
            </div>
        ...
        <form action="{{ route('users.store') }}" method="POST" class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <div>
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name') }}" class="input-field mt-2" required />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>
                <div>
                    <label for="username" class="block text-sm font-medium text-slate-700">Username</label>
                    <input id="username" name="username" type="text" value="{{ old('username') }}" class="input-field mt-2" required />
                    <x-input-error :messages="$errors->get('username')" class="mt-2" />
                </div>
                <div>
                    <label for="email" class="block text-sm font-medium text-slate-700">Email</label>
                    <input id="email" name="email" type="email" value="{{ old('email') }}" class="input-field mt-2" required />
                ...
                <div>
                    ...
                    <input id="password" name="password" type="password" class="input-field mt-2" required />
                    <x-input-error :messages="$errors->get('password')" class="mt-2" />
                </div>
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-slate-700">Konfirmasi Password</label>
                    <input id="password_confirmation" name="password_confirmation" type="password" class="input-field mt-2" required />
                </div>
            </div>
