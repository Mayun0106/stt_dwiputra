<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-slate-950">Edit Laporan</h2>
                <p class="mt-1 text-sm text-slate-500">Perbarui laporan yang sudah disimpan.</p>
            </div>
        ...
        <form action="{{ route('laporan.update', $laporan) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <div>
                    ...
                    <input id="judul" name="judul" type="text" value="{{ old('judul', $laporan->judul) }}" class="input-field mt-2" required />
                ...
                <div>
                    ...
                    <select id="tipe" name="tipe" class="input-field mt-2" required>
                        <option value="anggota" {{ old('tipe', $laporan->tipe) === 'anggota' ? 'selected' : '' }}>Anggota</option>
                        <option value="inventaris" {{ old('tipe', $laporan->tipe) === 'inventaris' ? 'selected' : '' }}>Inventaris</option>
                        <option value="kegiatan" {{ old('tipe', $laporan->tipe) === 'kegiatan' ? 'selected' : '' }}>Kegiatan</option>
                    </select>
                    <x-input-error :messages="$errors->get('tipe')" class="mt-2" />
                </div>
                <div>
                    <label for="tanggal_laporan" class="block text-sm font-medium text-slate-700">Tanggal Laporan</label>
                    <input id="tanggal_laporan" name="tanggal_laporan" type="date" value="{{ old('tanggal_laporan', optional($laporan->tanggal_laporan)->format('Y-m-d')) }}" class="input-field mt-2" required />
                ...
                <div>
                    ...
                    <select id="status" name="status" class="input-field mt-2" required>
                        ...
                        <option value="archived" {{ old('status', $laporan->status) === 'archived' ? 'selected' : '' }}>Archived</option>
                    </select>
            ...
            <div>
                ...
                <textarea id="konten" name="konten" rows="4" class="input-field mt-2">{{ old('konten', $laporan->konten) }}</textarea>
            ...
            <div>
                ...
                @if ($laporan->file_path)
                    <p class="mt-3 text-sm text-slate-500">File saat ini: <span class="font-semibold text-slate-900">{{ basename($laporan->file_path) }}</span></p>
                @endif
            </div>

            <div>
                <label for="keterangan" class="block text-sm font-medium text-slate-700">Keterangan</label>
                <textarea id="keterangan" name="keterangan" rows="4" class="input-field mt-2">{{ old('keterangan', $laporan->keterangan) }}</textarea>
                <x-input-error :messages="$errors->get('keterangan')" class="mt-2" />
            </div>

            <div class="flex justify-end">
                <button type="submit" class="btn-primary">Perbarui Laporan</button>
            </div>
        </form>
    </div>
</x-app-layout>
