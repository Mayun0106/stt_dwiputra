<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-slate-950">Tambah Anggota</h2>
                <p class="mt-1 text-sm text-slate-500">Buat catatan anggota baru untuk organisasi Anda.</p>
            </div>
        ...
        <form action="{{ route('anggota.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <div>
                    ...
                    <input id="email" name="email" type="email" value="{{ old('email') }}" class="input-field mt-2" />
                    <x-input-error :messages="$errors->get('email')" class="mt-2" />
                </div>
                <div>
                    <label for="nomor_hp" class="block text-sm font-medium text-slate-700">Nomor HP</label>
                    <input id="nomor_hp" name="nomor_hp" type="text" value="{{ old('nomor_hp') }}" class="input-field mt-2" />
                ...
                <div>
                    ...
                    <input id="tanggal_lahir" name="tanggal_lahir" type="date" value="{{ old('tanggal_lahir') }}" class="input-field mt-2" />
                    <x-input-error :messages="$errors->get('tanggal_lahir')" class="mt-2" />
                </div>
                <div>
                    <label for="jabatan" class="block text-sm font-medium text-slate-700">Jabatan</label>
                    <input id="jabatan" name="jabatan" type="text" value="{{ old('jabatan') }}" class="input-field mt-2" />
            ...
            <div>
                ...
                <textarea id="alamat" name="alamat" rows="3" class="input-field mt-2">{{ old('alamat') }}</textarea>
                <x-input-error :messages="$errors->get('alamat')" class="mt-2" />
            </div>

            <div>
                <label for="keterangan" class="block text-sm font-medium text-slate-700">Keterangan</label>
                <textarea id="keterangan" name="keterangan" rows="3" class="input-field mt-2">{{ old('keterangan') }}</textarea>
