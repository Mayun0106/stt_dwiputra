<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-2xl font-semibold text-slate-950">Edit Kegiatan</h2>
                <p class="mt-1 text-sm text-slate-500">Perbarui informasi kegiatan yang sudah ada.</p>
            </div>
        ...
        <form action="{{ route('kegiatan.update', $kegiatan) }}" method="POST" class="space-y-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <div>
                    ...
                    <input id="nama" name="nama" type="text" value="{{ old('nama', $kegiatan->nama) }}" class="input-field mt-2" required />
                    <x-input-error :messages="$errors->get('nama')" class="mt-2" />
                </div>
                <div>
                    <label for="tanggal_mulai" class="block text-sm font-medium text-slate-700">Tanggal Mulai</label>
                    <input id="tanggal_mulai" name="tanggal_mulai" type="date" value="{{ old('tanggal_mulai', optional($kegiatan->tanggal_mulai)->format('Y-m-d')) }}" class="input-field mt-2" required />
                    <x-input-error :messages="$errors->get('tanggal_mulai')" class="mt-2" />
                </div>
                <div>
                    <label for="tanggal_selesai" class="block text-sm font-medium text-slate-700">Tanggal Selesai</label>
                    <input id="tanggal_selesai" name="tanggal_selesai" type="date" value="{{ old('tanggal_selesai', optional($kegiatan->tanggal_selesai)->format('Y-m-d')) }}" class="input-field mt-2" />
                    <x-input-error :messages="$errors->get('tanggal_selesai')" class="mt-2" />
                </div>
                <div>
                    <label for="lokasi" class="block text-sm font-medium text-slate-700">Lokasi</label>
                    <input id="lokasi" name="lokasi" type="text" value="{{ old('lokasi', $kegiatan->lokasi) }}" class="input-field mt-2" />
                    <x-input-error :messages="$errors->get('lokasi')" class="mt-2" />
                </div>
                <div>
                    <label for="status" class="block text-sm font-medium text-slate-700">Status</label>
                    <select id="status" name="status" class="input-field mt-2" required>
                        <option value="upcoming" {{ old('status', $kegiatan->status) === 'upcoming' ? 'selected' : '' }}>Upcoming</option>
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4" class="input-field mt-2">{{ old('deskripsi', $kegiatan->deskripsi) }}</textarea>
                <x-input-error :messages="$errors->get('deskripsi')" class="mt-2" />
            </div>

            <div>
                <label for="keterangan" class="block text-sm font-medium text-slate-700">Keterangan</label>
                <textarea id="keterangan" name="keterangan" rows="4" class="input-field mt-2">{{ old('keterangan', $kegiatan->keterangan) }}</textarea>
